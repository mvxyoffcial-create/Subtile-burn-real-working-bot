...
async def start(client: Client, message: Message):
    await message.reply_text(
        ...
        "3. Use `/convert x264` or `/convert x265`."
    )
...
async def process_hardsub(client: Client, message: Message):
    ...
    ffmpeg_cmd = [
        "ffmpeg", "-y",
        "-i", video_file,
        "-vf", f"subtitles='{escaped_sub_file}':force_style='{sub_style}'",
        "-c:v", encoder,
        "-preset", "ultrafast",
        "-tune", "fastdecode",
        "-crf", "23",
    ...
    status_msg = await message.reply_text("⚡ **Starting High-Speed Hardsub Process...**")
